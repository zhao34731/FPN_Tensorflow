...
def test(frozen_graph_path, test_dir):
    ...
    cls = graph.get_tensor_by_name("detection_scores:0")
    score = graph.get_tensor_by_name("detections_classes:0")
    ...
    with tf.Session(graph=graph) as sess:
        for img_path in os.listdir(test_dir):
            ...
            st = time.time()
            boxes_det ,cls_det ,score_det = sess.run([boxes,cls,score], feed_dict={img: a_img})

            # show_indices = dets_val[:, 1] >= 0.5
            # dets_val = dets_val[show_indices]
            # final_detections = draw_box_in_img.draw_boxes_with_label_and_scores(a_img - cfgs.PIXEL_MEAN,
            #                                                                     boxes=dets_val[:, 2:],
            #                                                                     labels=dets_val[:, 0],
            #                                                                     scores=dets_val[:, 1])
            #
...
if __name__ == '__main__':
    test('/media/E/Develop/learn/FPN_Tensorflow/output/Pbs/FPN_Res101_SAR_revise_Frozen.pb',
         '/media/E/Develop/learn/FPN_Tensorflow/tools/demos')
